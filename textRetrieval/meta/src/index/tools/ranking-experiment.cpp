...
public:
    ...
    void set_param(double c, double lambda) {
        c_ = c;
        lambda_ = lambda;
    }; // Setter
...
double pl2_ranker::score_one(const index::score_data &sd) {
    ...
    */
    ...
    double score = (1 / (tfn + 1)) * (
            (tfn * (log2(tfn / lambda))) +
            (lambda + (1 / (12 * tfn)) - tfn) * log2(exp(1)) +
            ((0.5) * log2(2 * pi * tfn))
    );
...
void pl2_tune(const std::shared_ptr <index::dblru_inverted_index> &idx, std::vector <corpus::document> &allqueries,
              index::ir_eval &eval, double &c, double &lambda) {
    /*
    ...
    */
    ...
    double lambdavalues[8] = {0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1,
                              10}; // Different values for the parameter lambda
    ...
    for (int i = 0; i < 3; i++) // Loops over all c values
    {
        for (int j = 0; j < 8; j++) // Loops over all lambda values
        {
            ...
            ranker->set_param(cvalues[i],
                              lambdavalues[j]); // Sets the parameters of ranker to the current values of c and lambda
            for (std::vector<corpus::document>::iterator query = allqueries.begin();
                 query != allqueries.end(); ++query) // Iterates over all queries in allqueries
            {
                auto ranking = ranker->score(*idx, *query,
                                             1000); // Returns a ranked list of the top 1000 documents for the current query
                eval.avg_p(ranking, (*query).id(),
                           1000); // eval.avg_p stores the value of average precision for the current query in the instance eval
            }
            if (eval.map() >
    ...
    std::cout << "Max MAP = " << maxmap << " achieved by " << "c = " << cmax << ", lambda = " << lambdamax <<
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3 &&
        !(strcmp(argv[2], "task4") == 0 || strcmp(argv[2], "task5") == 0 || strcmp(argv[2], "task6") == 0 ||
          strcmp(argv[2], "task7") == 0)) {
    ...
    if (argc == 3) {
        ...
        if (!submission.is_open()) {
            std::cout <<
            "Problem writing the output to the system. Make sure the script has enough writing privileges. Quiting..." <<
            std::endl;
            return 0;
        }
        try {
