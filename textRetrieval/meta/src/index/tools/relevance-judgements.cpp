...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        { ranking = ranker->score(*idx, query, 20); });
        ...
        for (size_t i = 0; i < ranking.size() && i < 20; ++i)
        {
            ...
            std::cout << printing::make_bold(std::to_string(i + 1) + ". " + path) << std::endl;
        }
        std::cout<<"Enter the numbers of the relevant pages separated by spaces: " <<std::endl;
        int relevant_doc;
        bool success = 1;
        ...
        if (!success)
        {
            std::cout<<"Error: The relevance judgements should be valid numbers separated by spaces. Repeat your query"<<std::endl;
        }
        else
        {
        ...
        std::cout << "Enter another query to continue or a blank query to quit" << std::endl << std::endl;
    }
    qrels.close();
}
